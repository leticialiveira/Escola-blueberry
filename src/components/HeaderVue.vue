<script>
export default {
    data: () => ({
      items: [
        { title: 'Click Me' },
        { title: 'Click Me' },
        { title: 'Click Me' },
        { title: 'Click Me 2' },
      ],
    }),
  }
</script>

<template>
<div class="d-flex justify-space-around">
    <v-menu>
      <template v-slot:activator="{ props }">
        <v-btn
          color="primary"
          v-bind="props"
        >
          Nossa escola
        </v-btn>
      </template>
      <v-list>
        <v-list-item
          value="1"
        >
        <v-list-item-title>Endereço</v-list-item-title>
        <v-list-item-title>Estrutura</v-list-item-title>
        <v-list-item-title>Matricula</v-list-item-title>
        <v-list-item-title>Notas</v-list-item-title>
      </v-list-item>
      </v-list>
    </v-menu>

    <v-btn
      color="primary"
    >
    Formação 
      

      <v-menu activator="parent">
        <v-list>
          <v-list-item
            value="2"
          >
            <v-list-item-title>Maternal</v-list-item-title>
            <v-list-item-title>Infantil</v-list-item-title>
            <v-list-item-title>Ensino Fundamental</v-list-item-title>
            <v-list-item-title>Ensino Fundamental</v-list-item-title>
            <v-list-item-title>Formandos</v-list-item-title>
          </v-list-item>
        </v-list>
      </v-menu>
    </v-btn>

    <v-btn
      id="menu-activator"
      color="primary"
    >
      Contato
    </v-btn>

    <v-menu activator="#menu-activator">
      <v-list>
        <v-list-item
          value="3"
        >
          <v-list-item-title>Telefone</v-list-item-title>
          <v-list-item-title>Instagram</v-list-item-title>
          <v-list-item-title>Facebook</v-list-item-title>
          <v-list-item-title>WhatsApp</v-list-item-title>
        </v-list-item>
      </v-list>
    </v-menu>
  </div>
</template>

<style></style>
